...
class RobotEntry {
	...
	public RobotEntry(List<String> data) {
		...
		String uaString = it.next();
		if(uaString.startsWith("REGEX:")) {
			this.userAgentRegex = Pattern.compile(uaString.substring(6));
		} else {
			this.userAgentRegex = Pattern.compile(uaString, Pattern.LITERAL);
		}
	...
	public Pattern getUserAgentRegex() {
		return userAgentRegex;
	}

	public void setUserAgentRegex(Pattern userAgentRegex) {
